...
public class SmartUI : MonoBehaviour {
    ...
    public Vector2 screenPositionOfCenter()
    {
        return screenPosition() + getSize()/2;
    }
    ...
    Vector2 getScreenSpaceMultiplier(SmartUI sui, Vector2 currentMultiplier)
    {

        if(sui.unitTypes[2] == UnitTypes.pixel && sui.unitTypes[3] == UnitTypes.percentOfScreenHeight)
        if ((sui.unitTypes[2] == UnitTypes.percentOfParent && sui.unitTypes[3] == UnitTypes.percentOfParent) ||
            (sui.unitTypes[2] == UnitTypes.percentOfScreenWidth && sui.unitTypes[3] == UnitTypes.percentOfScreenHeight)) {
            currentMultiplier.x *= sui.position.width;
            currentMultiplier.y *= sui.position.height;
        }
        if (sui.unitTypes[2] == UnitTypes.percentOfScreenWidth && sui.unitTypes[3] == UnitTypes.percentOfScreenHeight)
            return currentMultiplier;
        if (sui.transform.parent != null)
        {
            SmartUI parentSUI = sui.transform.parent.GetComponent<SmartUI>();
            getScreenSpaceMultiplier(parentSUI, currentMultiplier);
        }
        return currentMultiplier;
    }

    public void setToScreenPoint(float pointXP, float pointYP)
    {
        Vector2 multiplier = getScreenSpaceMultiplier(this, new Vector2(1, 1));
        if (HAlign == HAligns.center)
        {
            position.x = (pointXP * multiplier.x) -(position.width / 2);
        }else
        {
            position.x = pointXP * multiplier.x;
        }
        if (HAlign == HAligns.center)
        {
            position.y = (pointYP * multiplier.y) - (position.height / 2);
        }else
        {
            position.y = pointYP * multiplier.y;
        }
    }
    ...
    public static float percentInScreenSpaceWidth(float val)
    {
        return val / Screen.width *100;
    }
    public static float percentInScreenSpaceHeight(float val)
    {
        return val / Screen.height *100;
    }
    ...
    public Vector2 getLocalPositionFromScreenSpace(float x, float y)
    {
        Vector2 ret = new Vector2(-1, -1);
        Vector2 v2 = getParentSize();
        ret.x = x / v2.x;
        ret.y = y / v2.y;
        return ret;
    }
